...
import { PropertyTypesEnum, ResourceProperty } from "./types";
...
export default function NextPropertyPicker({
  ...
  const handleSelectProperty = (property: ResourceProperty) => {
    ...
    const value = property.propertyType === PropertyTypesEnum.Element ? {} : null;
              ...
              {section.children.map(action => (
                <Box
                  ...
                  sx={{
                    ...
                    ":hover": {
                      background: !action.disabled ? "rgb(240,240,240)" : undefined,
                    },
                  }}
                >
                  <Box sx={{ display: "flex", flexGrow: 1 }}>{action.label}</Box>

                  {action.unsupported && <Box sx={{ color: "rgb(150,150,150)", fontSize: "12px" }}>Unsupported</Box>}

                  {action.baseChoiceType && (
                    <Box sx={{ color: "rgb(150,150,150)", fontSize: "12px" }}>{action.baseChoiceType}</Box>
                  )}
                </Box>
              ))}
            </Box>
