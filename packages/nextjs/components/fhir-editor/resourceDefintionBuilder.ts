...
import { PropertyTypesEnum, ResourceDefinitions, ResourceDefintion, ResourceProperty } from "./types";
...
export function getResourceDefintionAtPath(path: string, resourceDefinitions: ResourceDefinitions): ResourceDefintion {
...
function parseDefinition(
  ...
  function collectNestedElements(
    ...
    return (
      structureDefinition.snapshot?.element?.filter(element => {
        ...
        return pathLen.length + 1 === elementPath.length && element.path.startsWith(path);
      }) || []
    );
  }
  ...
  function getPropertyFromCode(
    ...
    if (!isInNestedArray && isArrayDef(definition)) {
      const nestedProperty = getPropertyFromCode(propertyName, elementType, definition, true);
    ...
    if (elementCode === "http://hl7.org/fhirpath/System.String" || elementCode === "string") {
      newProperty = {
      ...
      const nestedElementDefinitions = collectNestedElements(definition.path, structureDefinition);

      const properties = flatten(nestedElementDefinitions.map(transformElementDefinitionToProperty));
      ...
      if (!foundDef) {
        console.warn(`unable to find defintion for elementCode: ${elementCode}`);
        // else {
  ...
  function transformElementDefinitionToProperty(element: fhir4.ElementDefinition): ResourceProperty[] {
    ...
    if (!element.type || element.type.length === 0) {
      ...
      const elementType = element.type[0];

      const property = getPropertyFromCode(propertyName, elementType, element, false);
      if (property) {
    ...
    // ChoiceType element with multiple types
      ...
      if (!element.id?.endsWith("[x]")) {
        ...
        element.type.forEach(typeElement => {
          ...
          const nestedPropertyName = `${slicedPropertyName}${capitalizeFirstLetter(typeElement.code)}`;
          const property = getPropertyFromCode(nestedPropertyName, typeElement, element, false);
          if (property) {
            properties.push({
              ...property,
  ...
  const properties = flatten(firstTierElementDefinitions.map(transformElementDefinitionToProperty));
...
export function buildResourceDefintions(
  ...
  if (!foundResource || foundResource.resource?.resourceType !== "StructureDefinition") {
    throw new Error(`Unable to find structure definition with id '${structureDefinitionId}' in bundle`);
  }

  const structureDefinition = foundResource.resource as fhir4.StructureDefinition;

  return {
    rootPath: makeResourcePath(structureDefinition.name),
    resourceDefinitions: parseDefinition(structureDefinition, {}, 0, structureDefinitionBundle),
  };
}
