...
export default function CodeableConcept({ value, updateValue }: CodeableConceptProps) {
  ...
  return (
    <Box>
      <ListStyleWrapper>
          ...
          <Box component="li" className="dark">
            <Box sx={{ paddingBottom: "4px" }}>
              {editIndex === index ? (
                ...
                <CodingDisplay coding={codingRow} onEdit={() => startEditing(index)} />
              )}
            </Box>
          </Box>
        ))}
        ...
        {isAdding && (
          <Box component="li" className="dark">
            <CodingEdit initCoding={{}} onConfirmCoding={onAddCoding} onCancel={onCancelAdding} />
          </Box>
        )}
